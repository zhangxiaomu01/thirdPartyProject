...
namespace objl
{
	...
	public:
		...
		bool LoadFileSimple(std::string Path)
		{
			std::vector<Vertex> vertices;
			...
			for (int i = 0; i < temp_vertices.size(); ++i) {
				...
				//v.Normal = temp_normals[i];
				//v.TextureCoordinate = temp_uvs[i];
				vertices.push_back(v);
			}

			Mesh _mesh(vertices, vertexIndices);
			LoadedMeshes.emplace_back(_mesh);
			return true;
		}
